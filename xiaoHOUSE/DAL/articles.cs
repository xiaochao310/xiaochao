...
namespace xiaoHOUSE.DAL
{
    ...
    public partial class articles
    {
        ...
        public DataSet GetListByPage(string strWhere, string orderby, int pagesize, int pageindex, out int totalcout)
        {
            ...
            int endid = pagesize * pageindex;
            string sqlstr = "select count(1) from articles where" + (strWhere.Length > 0 ? strWhere : " 1=1");
            totalcout = (int)DbHelperSQL.GetSingle(sqlstr);
            sqlstr = "select top " + endid.ToString() + " * from articles where " + (strWhere.Length > 0 ? strWhere + " and ": strWhere ) + " ID not in (select  top " + startid.ToString() + "id from articles ";
            sqlstr += " where " + (strWhere.Length > 0 ? strWhere : " 1=1  ") + (orderby.Length > 0 ? orderby : "") + "  )" + (orderby.Length > 0 ? orderby : "");
